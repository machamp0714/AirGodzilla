...
class Api::V1::RoomsController < ApplicationController
  ...
  def create
    @room_registration_form =
      RoomRegistrationForm.new(
        room_registration_form_params.merge(user_id: current_user.id)
      )
    if @room_registration_form.save
      render json: { room: @room_registration_form }, status: :created
    else
      render json: { error: @room_registration_form.errors, is_success: false },
             status: 422
    end
  end
  ...
  def show
    ...
    if !room.nil?
      room_serializer =
        RoomSerializer.new(
          room,
          image: room.cover_photo, unavailable_dates: unavailable_dates
        )
